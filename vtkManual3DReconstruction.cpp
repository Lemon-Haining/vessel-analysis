...
int vtkManual3DReconstruction()
{
	...
	//appendFilter->AddInputConnection(tube01->GetOutputPort());
	//appendFilter->AddInputConnection(tube12->GetOutputPort());
	//appendFilter->AddInputConnection(tube23->GetOutputPort());
	//appendFilter->AddInputConnection(tube24->GetOutputPort());
	//appendFilter->AddInputConnection(tube45->GetOutputPort());
	//appendFilter->AddInputConnection(tube46->GetOutputPort());  // vessel_test.stl is the tube24,tube45 and tube46
	//appendFilter->AddInputConnection(tube17->GetOutputPort());
	appendFilter->AddInputConnection(tube78->GetOutputPort());  // tube78 will be tested for 3D reconstruction 
	//appendFilter->AddInputConnection(tube79->GetOutputPort());
	//appendFilter->AddInputConnection(tube910->GetOutputPort());
	//appendFilter->AddInputConnection(tube1011->GetOutputPort());

	vtkSmartPointer<vtkCleanPolyData> cleanFilter = vtkSmartPointer<vtkCleanPolyData>::New();
	cleanFilter->SetInputConnection(appendFilter->GetOutputPort());
	cleanFilter->Update();

	vtkSmartPointer<vtkPolyDataMapper>filterMapper = vtkSmartPointer<vtkPolyDataMapper>::New();
	filterMapper->SetInputConnection(cleanFilter->GetOutputPort());

	vtkSmartPointer<vtkSTLWriter> stlWriter1011 = vtkSmartPointer<vtkSTLWriter>::New();
	stlWriter1011->SetFileName("./ReconstructionStl/vesselCylinder_78.stl");  // F:\CASIA\codes\Project_test1\ReconstructionStl
	stlWriter1011->SetInputConnection(cleanFilter->GetOutputPort());
	//stlWriter1011->SetFileTypeToBinary();
	stlWriter1011->Write();
#if 0
	/*另一种合并的方法，会考虑其中相交的部分*/
	vtkSmartPointer<vtkBooleanOperationPolyDataFilter> m_BooleanFilter = vtkBooleanOperationPolyDataFilter::New();;
	vtkSmartPointer<vtkPolyDataNormals> m_Normal = vtkSmartPointer<vtkPolyDataNormals>::New();
	vtkSmartPointer<vtkTriangleFilter> m_TriFilter1 = vtkSmartPointer<vtkTriangleFilter>::New();
	vtkSmartPointer<vtkTriangleFilter> m_TriFilter2 = vtkSmartPointer<vtkTriangleFilter>::New();
	...
	m_TriFilter1->SetInputConnection(tube24->GetOutputPort());
	m_TriFilter2->SetInputConnection(tube45->GetOutputPort());
	...
	m_BooleanFilter->SetOperationToUnion();
	...
	vtkSmartPointer<vtkSTLWriter> stlWriter_test = vtkSmartPointer<vtkSTLWriter>::New();
	stlWriter_test->SetFileName("./ReconstructionStl/vessel_24_45_test.stl");  // F:\CASIA\codes\Project_test1\ReconstructionStl
	stlWriter_test->SetInputConnection(m_Normal->GetOutputPort());
	//stlWriter1011->SetFileTypeToBinary();
	stlWriter_test->Write();
#endif
...
void vtkManual3DReconstructionRadious()
{
	...
	vtkSmartPointer<vtkParametricFunctionSource>splineSource78 = vtkSmartPointer<vtkParametricFunctionSource>::New();
	splineSource78->SetParametricFunction(spline78);
	splineSource78->SetUResolution(10 * point78->GetNumberOfPoints());  //这里是每两个点之间插入了10个点，为后面的拟合做准备
	...
	tube78->SetNumberOfSides(24);
	tube78->SetRadius(2.5);
	tube78->CappingOn();  // 加个盖子，因为流体动力学分析软件 ANSYS ICEM cfd 需要知道入口和出口*/
	...
	rad = 2.5;  interpolatedRadius->AddTuple(0, &rad);
	rad = 2.5 * 0.8; interpolatedRadius->AddTuple(1, &rad);
	rad = 2.5 * 0.6; interpolatedRadius->AddTuple(2, &rad);
	rad = 2.5 * 0.8; interpolatedRadius->AddTuple(3, &rad);
	rad = 2.5; interpolatedRadius->AddTuple(4, &rad);
	//对于半径进行插值准备，准备好样条曲线插值的点的参数
	vtkSmartPointer<vtkDoubleArray> tubeRadius = vtkSmartPointer<vtkDoubleArray>::New();
	unsigned int n = splineSource78->GetOutput()->GetNumberOfPoints();
	...
	{
		double t = (tMax - tMin) / (n - 1) * i + tMin;  // 这个应该是设置准备插值的点对应的序列参数t
		interpolatedRadius->InterpolateTuple(t, &r);  // 这里应该是对于每一个点对应的半径进行样条插值计算
		tubeRadius->SetTuple1(i, r);  // 这个应该是根据半径生成管道的圆
	}
	
	//这里应该是生成了一个点云图，但是只是一圈一圈的圆的点云
	vtkSmartPointer<vtkPolyData> tubePolyData = vtkSmartPointer<vtkPolyData>::New();
	tubePolyData = splineSource78->GetOutput();
	tubePolyData->GetPointData()->AddArray(tubeRadius);
	tubePolyData->GetPointData()->SetActiveScalars("TubeRadius");
	//根据圆的点云进行插值，生成圆柱
	vtkSmartPointer<vtkTubeFilter>tube78 = vtkSmartPointer<vtkTubeFilter>::New();
	tube78->SetInputData(tubePolyData);
	tube78->SetNumberOfSides(50);
	tube78->SetVaryRadiusToVaryRadiusByAbsoluteScalar();
	tube78->CappingOn();
	
	vtkSmartPointer<vtkSTLWriter> stlWriter78 = vtkSmartPointer<vtkSTLWriter>::New();
	stlWriter78->SetFileName("./ReconstructionStl/vesselRadious_78.stl");  // F:\CASIA\codes\Project_test1\ReconstructionStl
	stlWriter78->SetInputConnection(tube78->GetOutputPort());
	//stlWriter1011->SetFileTypeToBinary();
	stlWriter78->Write();
	...
	tubeMapper->SetInputConnection(tube78->GetOutputPort());
